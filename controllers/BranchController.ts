...
import SubSites from "../models/SubSites";
...
const createNewBranch = (req: Request, res: Response) => {
  const { branchEmail, location }: branchObjectInterface = req.body;
  Branch.create<{ branchEmail: string; location: string | undefined }>({
    branchEmail,
    location,
  })
    .then(async (data) => {
      try {
        ...
        for await (let subsites of allAffiliates.data as Array<affiliatesListInterface>) {
          try {
            const subSite1 = await SubSites.create<loObjectInterface>({
              branchId: data._id,
...
const postBlogToDb = async (
  req: GenericExpressPostRequestInterface<InterFaceRequestBlogPostMeta>,
  res: Response
) => {
  const {
    postId,
  ...
  BlogPost.create<InterfaceRequestBlog | string | number>({
    siteId,
    siteType,
    postContent,
    websiteUrl,
    ...
    .then(async (result) => {
      try {
        if (
          ...
          await PostMeta.create<blogPostMetaType, string | number>({
            postId: result?._id,
            elementorEditMode,
            elementorTemplateType,
            elementorVersion,
